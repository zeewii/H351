...
#输出：输入命令返回的结果
def telnet_command(user,ip,password1,password2,commands):
    try:
        ...
        #出现登录提示，输入用户名
        child.read_until('Username:')
        child.write(user+'\n')

        #出现密码提示，输入密码
        child.read_until('Password:')
        child.write(password1+'\n')

        child.read_until('router>')
        child.write('sh\n')

        child.read_until('Password:')
        child.write(password2+'\n')
...
if __name__ == '__main__':
    ...
    password1 = '100msh'
    password2 = '@w$r^y*i90'
    commands = 'ifconfig br-lan'
    result = telnet_command(user,ip,password1,password2,commands)
    print result